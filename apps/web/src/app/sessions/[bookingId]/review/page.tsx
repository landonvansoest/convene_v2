...
export default function SessionReviewPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setBusy(true);
    setErr(null);
    const res = await fetch(`/api/sessions/${encodeURIComponent(bookingId)}/reviews/expert`, {
      method: "POST",
  ...
  return (
    <div className="min-h-screen bg-[var(--convene-primary)] px-4 py-10 text-white">
      <div className="mx-auto max-w-md">
        ...
        <h1 className="mt-4 text-2xl font-semibold">Review expert</h1>
        <p className="mt-2 text-sm text-white/75">
          For completed sessions only. One review per booking.
        </p>
        ...
        <form onSubmit={(e) => void onSubmit(e)} className="mt-8 space-y-4">
          ...
          <label className="block">
            <span className="text-xs text-white/80">Private note to expert (optional)</span>
            <textarea
              rows={2}
              className="mt-1 w-full rounded-md border border-white/25 bg-black/25 px-3 py-2 outline-none focus:border-[var(--convene-hero)]"
              value={privateMsg}
