...
- [ ] **Stripe Connect required for bookings** — Vercel Preview currently bypasses the expert `stripe_connect_account_id` gate (`VERCEL_ENV=preview` in `isSessionPaymentTestBypassAllowed`). **Before launch:** remove that preview bypass (or confirm production never sets `VERCEL_ENV=preview`), set `ALLOW_PAYMENT_BYPASS=false`, turn OFF Admin → DEV Tools → `payment_bypass_session`, and verify booking an expert without completed Stripe Connect payout setup returns **“Expert payment setup not complete”** instead of allowing checkout.
